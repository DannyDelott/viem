...
export type EIP1474Methods = [...PublicRpcMethods, ...WalletRpcMethods]

export type EIP1193Provider = Events & {
  request: RpcRequestFn<EIP1474Methods, { Strict: false }>
}
...
export type TestRpcMethods<TMode extends TestClientMode = TestClientMode> = [
  /**
...
export type RpcMethods = (
  | { Method: string; Parameters?: undefined; ReturnType: unknown }
  | { Method: string; Parameters?: unknown; ReturnType: unknown }
)[]

type RpcRequestFnConfig = { Strict: boolean }

type InferMethod<TRpcMethods extends RpcMethods, TStrict> = TStrict extends true
  ? TRpcMethods[number]['Method']
  : TRpcMethods[number]['Method'] | (string & {})

type InferParameters<
  TRpcMethods extends RpcMethods,
  TMethod extends string,
> = Extract<TRpcMethods[number], { Method: TMethod }>['Parameters']

type GetParameters<
  TRpcMethods extends RpcMethods,
  TMethod extends string,
> = InferParameters<TRpcMethods, TMethod> extends never
  ? { params?: unknown }
  : InferParameters<TRpcMethods, TMethod> extends undefined
  ? { params?: never }
  : {
      params: InferParameters<TRpcMethods, TMethod>
    }

type InferReturnType<
  TRpcMethods extends RpcMethods,
  TMethod extends string,
> = Extract<TRpcMethods[number], { Method: TMethod }>['ReturnType']

type GetReturnType<
  TRpcMethods extends RpcMethods,
  TMethod extends string,
> = InferReturnType<TRpcMethods, TMethod> extends undefined
  ? unknown
  : InferReturnType<TRpcMethods, TMethod>

export type RpcRequestFn<
  TRpcMethods extends RpcMethods = RpcMethods,
  TConfig extends RpcRequestFnConfig = { Strict: true },
> = <
  TMethod extends InferMethod<TRpcMethods, TConfig['Strict']> = InferMethod<
    TRpcMethods,
    TConfig['Strict']
  >,
>(
  args: {
    method: TRpcMethods extends RpcMethods ? TMethod : string
  } & GetParameters<TRpcMethods, TMethod>,
) => Promise<GetReturnType<TRpcMethods, TMethod>>
